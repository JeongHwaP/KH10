...
public class PayTest08 {
	...
	public void test() {
		...
		//1. 모든 payment를 조회
		List<PaymentDto> list = paymentDao.paymentHistory("hello1236");
		System.out.println(list.size());
		for(PaymentDto paymentDto : list) {
			//2. payment에 소속된 payment_detail을 조회
			List<PaymentDetailDto> subList = 
				paymentDao.findPaymentDetail(paymentDto.getPaymentNo());
